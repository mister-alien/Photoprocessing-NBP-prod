...
def renstuff( spreadsheet, directory, namecol, platecol, txcol, prefix, offset ):
   ...
   # platecol='E'
    ...
    if not os.path.exists(directory+'\\..\\processed\\'):
        ...
        shutil.rmtree(directory+'\\..\\processed\\')
        os.makedirs(directory+'\\..\\processed\\')
    for x in range(1,n-offset):
        ...
        if (platenum.value) and (txnum.value):
            shutil.copy(directory+'\\'+prefix[0:(len(prefix)-len(str(platenum.value)))]+str(platenum.value)+'.jpg',directory+'\\..\\processed\\'+name.value+' P.jpg')
#           os.rename('raw\\'+prefix[0:(len(prefix)-len(str(platenum.value)))]+str(platenum.value)+'.jpg',name.value+' P.jpg')
            shutil.copy(directory+'\\'+prefix[0:(len(prefix)-len(str(txnum.value)))]+str(txnum.value)+'.jpg',directory+'\\..\\processed\\'+name.value+' T.jpg')
...
#               os.rename('raw\\'+prefix[0:(len(prefix)-len(str(platenum.value)))]+str(platenum.value)+'.jpg',name.value+'.jpg')
                shutil.copy(directory+'\\'+prefix[0:(len(prefix)-len(str(platenum.value)))]+str(platenum.value)+'.jpg',directory+'\\..\\processed\\'+name.value+'.jpg')
                Tri = 'C'
            elif Tri=='C':
#               os.rename('raw\\'+prefix[0:(len(prefix)-len(str(platenum.value)))]+str(platenum.value)+'.jpg',name.value+'.jpg')
                shutil.copy(directory+'\\'+prefix[0:(len(prefix)-len(str(platenum.value)))]+str(platenum.value)+'.jpg',directory+'\\..\\processed\\'+name.value+'.jpg')
